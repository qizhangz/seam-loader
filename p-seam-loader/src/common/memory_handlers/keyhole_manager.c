...
_STATIC_INLINE_ uint64_t la_from_keyhole_idx(uint16_t keyhole_idx)
{
    ...
    pseamldr_debug_assert(keyhole_idx < MAX_KEYHOLE_PER_LP);

    return get_psysinfo_table()->keyhole_rgn_base + (uint64_t)(keyhole_idx * 0x1000);
}
...
_STATIC_INLINE_ uint16_t keyhole_idx_from_la(uint64_t la)
{
    ...
    pseamldr_debug_assert(la >= (get_psysinfo_table()->keyhole_rgn_base));
    pseamldr_debug_assert(la < (get_psysinfo_table()->keyhole_rgn_base + get_psysinfo_table()->keyhole_rgn_size));

    uint16_t keyhole_idx = (uint16_t)(((la - get_psysinfo_table()->keyhole_rgn_base) / 0x1000));
...
{
    uint64_t lp_keyhole_edit_base = get_psysinfo_table()->keyhole_edit_rgn_base;
...
{
    keyhole_state_t* keyhole_state = &get_pseamldr_data()->keyhole_state;
...
static void hash_table_remove_entry(uint64_t pa, bool_t is_writable, bool_t is_wb_memtype)
{
    ...
    {
        pseamldr_debug_assert(prev_idx < MAX_KEYHOLE_PER_LP);
...
static void lru_cache_remove_entry(uint16_t keyhole_idx)
{
    ...
    {
        pseamldr_debug_assert(lru_prev < MAX_KEYHOLE_PER_LP);
        keyhole_state->keyhole_array[lru_prev].lru_next = lru_next;
    }

    if (lru_next != UNDEFINED_IDX)
    {
        pseamldr_debug_assert(lru_next < MAX_KEYHOLE_PER_LP);
...
static void* map_pa_with_memtype(void* pa, mapping_type_t mapping_type, bool_t is_wb_memtype)
{
    ...
    // Check if there any available keyholes left, otherwise - kill the module or something...
    pseamldr_sanity_check(keyhole_idx != UNDEFINED_IDX, SCEC_KEYHOLE_MANAGER_SOURCE, 0);
                           ...
                           uint16_t starting_static_keyhole)
{
    ...
    pseamldr_debug_assert(MAX_STATIC_KEYHOLES > starting_static_keyhole);
    pseamldr_debug_assert(array_size <= (MAX_STATIC_KEYHOLES - starting_static_keyhole));
