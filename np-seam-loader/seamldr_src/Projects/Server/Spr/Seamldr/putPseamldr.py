...
ifwi_image = sys.argv[1]
bios_file = sys.argv[2]
new_ifwi = sys.argv[3]

f_ifwi = open(ifwi_image, 'rb')
f_bios = open(bios_file, 'rb')
f_newifwi = open(new_ifwi, 'wb')

pre_data = f_ifwi.read(0x44)
f_newifwi.write(pre_data)
bios_loc = f_ifwi.read(4)
f_newifwi.write(bios_loc)
bios_loc_int, bios_loc_end = struct.unpack("HH", bytearray(bios_loc))
bios_loc_int *= 0x1000
bios_loc_end += 1
bios_loc_end *= 0x1000
print hex(bios_loc_int)
print hex(bios_loc_end)
f_newifwi.write(f_ifwi.read(bios_loc_int - 0x48))
f_newifwi.write(f_bios.read())


#bios_data = f_bios.read()
#f_out.write(pre_data)
#f_out.write(module_data)
#f_bios.seek(offset + len(module_data))
#f_out.write(f_bios.read())
